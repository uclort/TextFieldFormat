//
//  HmPhoneTextField.swift
//  chowRent
//
//  Created by HouMeng on 2019/1/23.
//  Copyright © 2019 eallcn. All rights reserved.
//

import UIKit

class HmPhoneTextField: UITextField, UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if string == "" {
            ...
            if self._noneSpaseString(string: textField.text! as NSString).count + string.count - range.length > 11 {
                return false
            }
    ...
    private func _parseString(string: NSString) -> String {
        ...
        if mStr.length > 3 {
            mStr.insert(placeholderSpace as String, at: 3)
        }
        if mStr.length > 8 {
            mStr.insert(placeholderSpace as String, at: 8)
        }
